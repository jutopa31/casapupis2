...
import { ImageIcon, Camera, Plane } from 'lucide-react';
...
const TABLE = 'fotos_luna_de_miel';
const BUCKET = 'fotos-luna-de-miel';
...
export default function FotosLunaDeMielPage() {
  ...
  const loadPhotos = useCallback(async (offset = 0, append = false) => {
    ...
    try {
      ...
      console.error('Error cargando fotos de la luna de miel:', err);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('fotos_luna_de_miel_realtime')
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  return (
    <div className="min-h-screen bg-[var(--color-offwhite)] pb-24 md:pb-8">
      ...
      <header className="px-4 pt-8 pb-2 text-center sm:pt-12">
        ...
        >
          <Plane size={20} style={{ color: '#C9A84C' }} />
        ...
        >
          Compartí los recuerdos de nuestra luna de miel en Colonia
        </motion.p>
      </header>
        ...
        {!isLoading && !error && photos.length === 0 && (
          ...
          >
            ...
            <p className="mt-4 text-base font-medium text-stone-600">
              Todavia no hay fotos de la luna de miel
            </p>
            <p className="mt-1 text-sm text-stone-400">
              ¡Compartí los primeros recuerdos de Colonia de Sacramento!
            </p>
          </motion.div>
        )}
