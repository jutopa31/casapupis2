...
interface MilestoneEditModalProps {
  milestone: Partial<HistoriaMilestone> | null
  nextOrden: number
...
function isValidSpotifyUrl(url: string): boolean {
  try { return new URL(url).hostname === 'open.spotify.com' }
  catch { return false }
}

export default function MilestoneEditModal({ milestone, nextOrden, onSave, onClose }: MilestoneEditModalProps) {
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const compressed = await imageCompression(file, {
        maxSizeMB: 2, maxWidthOrHeight: 2048, useWebWorker: true, initialQuality: 0.85,
      })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-stone-100 bg-white px-5 py-4 rounded-t-2xl">
          <h3 className="font-serif text-lg text-stone-800">{isNew ? 'Nuevo momento' : 'Editar momento'}</h3>
          <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full text-stone-400 transition-colors hover:bg-stone-100 hover:text-stone-600">
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="p-5 space-y-5">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-stone-500 uppercase tracking-wider">Foto</label>
            {imageUrl ? (
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={isUploading}
                ...
                <span className="text-sm font-medium">{isUploading ? 'Subiendo...' : 'Subir foto'}</span>
              </button>
            )}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-stone-500 uppercase tracking-wider">Titulo *</label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ej: Nos conocimos"
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-stone-500 uppercase tracking-wider">Descripcion</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Cuenta la historia de este momento..." rows={3}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-stone-500 uppercase tracking-wider">Fecha</label>
            <input type="text" value={date} onChange={(e) => setDate(e.target.value)} placeholder="Ej: Marzo 2020 o 2020-03-15"
              className="w-full rounded-xl border border-stone-200 px-4 py-3 text-sm text-stone-800 placeholder:text-stone-400 focus:border-[#C9A84C] focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/20" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-stone-500 uppercase tracking-wider">Cancion de Spotify</label>
            <div className="relative">
              <Music className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" />
              <input type="url" value={spotifyUrl} onChange={(e) => setSpotifyUrl(e.target.value)} placeholder="https://open.spotify.com/track/..."
            ...
            {showSpotifyPreview && (
              <div className="mt-2 rounded-lg border border-stone-100 bg-stone-50 p-2">
                <iframe src={toSpotifyEmbedUrl(spotifyUrl)} width="100%" height="80"
                  allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
                  loading="lazy" className="rounded-md border-0" title="Spotify preview" />
              </div>
            )}
          </div>
        </div>
        <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-stone-100 bg-white px-5 py-4 rounded-b-2xl">
          <button onClick={onClose} className="rounded-xl px-5 py-2.5 text-sm font-medium text-stone-500 transition-colors hover:bg-stone-100">Cancelar</button>
          <button onClick={handleSave} disabled={!title.trim() || isSaving || isUploading}
            ...
            {isSaving ? <><Loader2 className="h-4 w-4 animate-spin" />Guardando...</> : (isNew ? 'Crear' : 'Guardar')}
          </button>
        </div>
      </motion.div>
    </motion.div>
